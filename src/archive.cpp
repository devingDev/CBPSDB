...
int extractArchiveFile(const char *src_path, const char *dst_path) {
...
int extractArchivePath(const char *src_path, const char *dst_path) {
  ...
  if (SCE_S_ISDIR(stat.st_mode)) {
    ...
    for (i = 0; i < list.length - 1; i++, entry = entry->next) {
      ...
      if (entry->is_folder) {
        ret = extractArchivePath(new_src_path, new_dst_path);
      } else {
        ret = extractArchiveFile(new_src_path, new_dst_path);
      }
    ...
    return extractArchiveFile(src_path, dst_path);
  }
